...
def generate_flight_trajectory(df, ts_str, output_dir: Path):
    ...
    ref_ias_col = next((col for col in ['AP_Target_Speed_Kts', 'Ref_IAS', 'Ref_Speed', 'AP_Target_Speed'] if col in df.columns), None)
    # Check for valid actual IAS telemetry (> 0)
    has_valid_ias = ias_col is not None and (df[ias_col].dropna() != 0).any()
    # Screen target speed: must exist, not be empty, and not be stuck entirely at 0 or 100
    has_valid_target = (
        ref_ias_col is not None 
        and not df[ref_ias_col].dropna().empty 
        and (df[ref_ias_col].dropna() != 0).any()
        and (df[ref_ias_col].dropna() != 100).any()
    )
...
def generate_scorecard_dashboard(aln_df, sc_df, ts_str, output_dir: Path):
    ...
    pitch_inputs = aln_df['Yoke_Pitch_Pct'].dropna().values if 'Yoke_Pitch_Pct' in aln_df.columns else np.array([])
    ...
    tiles = [
        (0.05, 0.58, "ALT TOL", sess_tol_alt, COLORS['Altitude']),
        (0.52, 0.58, "HDG TOL", sess_tol_hdg, COLORS['Heading']),
        (0.05, 0.27, "BANK TOL", sess_tol_bnk, COLORS['Bank']),
        (0.52, 0.27, "COMBINED", sess_tol_env, COLORS['Pass'] if env_pass else COLORS['Fail'])
    ]
    for x_pos, y_pos, label, val, color in tiles:
        # Draw uniform background sub-tile box using FancyBboxPatch
        rect = FancyBboxPatch(
            (x_pos, y_pos), 0.43, 0.27,
        ...
        # Label Header
        ax_kpi_req.text(x_pos + 0.04, y_pos + 0.17, label, color='#888888', fontsize=9, fontweight='bold', transform=ax_kpi_req.transAxes)
