...
class ReferenceTrajectoryGenerator:
    ...
    def __init__(
        self, max_bank=30.0, roll_rate=5.0, default_ias_target=300.0, vsi_ramp_time=4.0, reaction_delay=1.5
    ):
        self.max_bank = max_bank # deg
        self.roll_rate = roll_rate # deg/s
        self.default_ias_target = default_ias_target  # kt Central default IAS if none provided
    ...
    def calculate_turn_rate(
        ...
        if current_ias is None:
            current_ias = self.default_ias_target
    ...
    def generate_trajectory(
        ...
        for i in range(n):
            ...
            # -------------------------------------------------------------
            # VSI & Altitude Kinematics (Standard 10% VSI Capture Lead)
            # -------------------------------------------------------------
            alt_diff = target_alt - curr_alt
            # Standard flight lead distance: 10% of current VSI (ft)
            lead_alt = max(0.10 * abs(curr_vsi), 25.0)

            if abs(alt_diff) <= 1.0 and abs(curr_vsi) <= 10.0:
                curr_alt = target_alt
                desired_vsi = 0.0
            elif abs(alt_diff) <= lead_alt and (alt_diff * curr_vsi > 0):
                # Parabolic pitch deceleration during altitude level-off
                vsi_dir = np.sign(alt_diff)
                desired_vsi = vsi_dir * abs(target_vsi) * (abs(alt_diff) / lead_alt)
            else:
