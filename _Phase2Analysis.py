...
def normalize_telemetry_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filter single-frame 0.0 telemetry dropouts across all numeric metric columns
    for col in df.select_dtypes(include=[np.number]).columns:
        is_dropout = (df[col] == 0.0) & (df[col].shift(1).abs() > 5.0)
        df.loc[is_dropout, col] = np.nan
...
class FlightPerformanceAnalyzer:
    ...
    def process_and_align(
        ...
        # Oscillation Flag: Flying opposite to intended direction
        directional_violation = (is_climbing & (merged["VSI"] < -50.0)) | (is_descending & (merged["VSI"] > 50.0))
        # Rate Stability Flag: Excessive VSI acceleration/hunting
        vsi_rate_of_change = np.abs(np.gradient(merged["VSI"], dt))
        vsi_unstable = vsi_rate_of_change > 300.0
        # 3. Collapse Corridor using Pandas .where() (Preserves Index & Series Type)
        corridor_active = ~(directional_violation | vsi_unstable)
        effective_ref_min = ref_alt_min.where(corridor_active, merged["Ref_Alt"])
        effective_ref_max = ref_alt_max.where(corridor_active, merged["Ref_Alt"])
        # 4. Calculate Final Altitude Error
        below_mask = merged["Altitude"] < effective_ref_min
        above_mask = merged["Altitude"] > effective_ref_max
        merged["Alt_Err"] = 0.0
        merged.loc[below_mask, "Alt_Err"] = merged.loc[below_mask, "Altitude"] - effective_ref_min[below_mask]
        merged.loc[above_mask, "Alt_Err"] = merged.loc[above_mask, "Altitude"] - effective_ref_max[above_mask]
